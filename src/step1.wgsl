...
// construct bitmap for positions of structural characters
fn extractNibbleFromWordStructural(word: u32) -> u32 {
    let b0 = word & 0xFFu;
    let b1 = (word >> 8u)  & 0xFFu;
    let b2 = (word >> 16u) & 0xFFu;
    let b3 = (word >> 24u) & 0xFFu;
    let is0 = u32(b0 == COMMA || b0 == DOUBLE_COLON || b0 == BRACE_OPEN || b0 == BRACE_CLOSE || b0 == BRACKET_OPEN || b0 == BRACKET_CLOSE);   // bit 0
    let is1 = u32(b1 == COMMA || b1 == DOUBLE_COLON || b1 == BRACE_OPEN || b1 == BRACE_CLOSE || b1 == BRACKET_OPEN || b1 == BRACKET_CLOSE);   // bit 0
    let is2 = u32(b2 == COMMA || b2 == DOUBLE_COLON || b2 == BRACE_OPEN || b2 == BRACE_CLOSE || b2 == BRACKET_OPEN || b2 == BRACKET_CLOSE);   // bit 0
    let is3 = u32(b3 == COMMA || b3 == DOUBLE_COLON || b3 == BRACE_OPEN || b3 == BRACE_CLOSE || b3 == BRACKET_OPEN || b3 == BRACKET_CLOSE);   // bit 0
...
fn extractNibbleFromWordBackslash(word: u32) -> u32 {
    ...
    let b2 = (word >> 16u) & 0xFFu;
    let b3 = (word >> 24u) & 0xFFu;
    let is0 = u32(b0 == BACKSLASH);   // bit 0
    let is1 = u32(b1 == BACKSLASH);   // bit 0
    let is2 = u32(b2 == BACKSLASH);   // bit 0
    let is3 = u32(b3 == BACKSLASH);   // bit 0
...
fn extractNibbleFromWordDoubleQuote(word: u32) -> u32 {
    ...
    let b2 = (word >> 16u) & 0xFFu;
    let b3 = (word >> 24u) & 0xFFu;
    let is0 = u32(b0 == DOUBLE_QUOTE);   // bit 0
    let is1 = u32(b1 == DOUBLE_QUOTE);   // bit 0
    let is2 = u32(b2 == DOUBLE_QUOTE);   // bit 0
    let is3 = u32(b3 == DOUBLE_QUOTE);   // bit 0
